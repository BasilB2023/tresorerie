...
frappe.ui.form.on('Caisse Initialisation', {
		...
		frm.add_custom_button(
			...
			function () {
				let d = new frappe.ui.Dialog({
					...
					primary_action(values) {
						frappe.call({
							method: 'tresorerie.tresorerie.doctype.caisse_initialisation.caisse_initialisation.save_operation',
							args: {
	...
	init_billetage: function(frm) {
        frappe.call({
			method: "tresorerie.tresorerie.doctype.devise.devise.get_billetage",
			args: {
	...
	get_billetage: function(frm,values) {
		...
		frappe.call({
			method: "tresorerie.tresorerie.doctype.caisse_initialisation.caisse_initialisation.get_solde_final",
			args: {
				caisse : values.caisse,
				date: values.date,
			},
			callback: function (r) {
				...
				if (r.message) {
					...
					frappe.call({
						method: "tresorerie.tresorerie.doctype.caisse_initialisation.caisse_initialisation.get_last_billetage",
						args: {
							caisse : values.caisse,
							date: values.date,
						},
	...
	cours: function(from, to) {
		frappe.call({
			method: "tresorerie.tresorerie.doctype.devise.devise.get_cours",
			args: {
				reference: from,
				devise: to,
			},
	...
	devise: function(caisse) {
		frappe.call({
			method: "tresorerie.tresorerie.doctype.caisse_initialisation.caisse_initialisation.get_caisse_devise",
			args: {
				caisse: caisse,
			},
			callback: function (r) {
