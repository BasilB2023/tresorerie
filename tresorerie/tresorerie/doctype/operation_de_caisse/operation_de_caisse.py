...
from tresorerie.tresorerie.doctype.devise.devise import get_cours

#from tresorerie.tresorerie.doctype.decaissement.decaissement import get_decaissement_route
...
class OperationdeCaisse(Document):
	...
	def before_save(self):
		...
		if exist:
			frappe.throw("Vous ne pouvez enregistrer d'opérations sur cette jounéee: " + self.initialisation + ". Veuillez choisir une journée valide!")
		...
		if date_split != str(self.date):
			frappe.throw("La date de saisie " + str(self.date) + " doit être conforme à la date d'ouverture " + date_split)
