...
frappe.ui.form.on('Decaissement', {
	...
	refresh(frm) {
		frm.add_custom_button('Demandes de Paiment',() =>{ 
			let query_args = {
				//query:"tresorerie.tresorerie.doctype.decaissement.decaissement.get_demande_paiement",
				filters: { 
	...
	/*after_save: function(frm){
		if(frm.is_new()) return;
		frappe.call({
			method: "tresorerie.tresorerie.doctype.operation_de_caisse.operation_de_caisse.insert_operation",
			args: {
				doc: frm.doc,
				type: 2,
			},
